...
export async function findUserByLogin(usuario, senha) {
  ...
  if (upperUser !== normalizedUser) {
    const upperCached = await getJsonCache(loginCacheKey(upperUser, senhaHash));
    if (upperCached) return normalizeUser(upperCached);
  }
  ...
  if (!canUseDirectFirebird()) {
    ...
    if (upperUser !== normalizedUser) {
      const upperStale = await getStaleJsonCache(loginCacheKey(upperUser, senhaHash));
      if (upperStale.hit) return normalizeUser(upperStale.value);
    }

...
export async function getDashboardAccess(baseBranch, employeeCode, options = {}) {
  ...
  const allBranchesCacheKey = createCacheKey("dashboard-access-all", {
    version: 1,
  });

  if (!CODIGO_FILIAL_VALIDO.test(filial) || !funcionario) {
    const access = await getAllBranchesAccess(allBranchesCacheKey, options);
...
async function getAllBranchesAccess(cacheKey, options = {}) {
  if (!options.forceRefresh) {
    const cached = await getJsonCache(cacheKey);
    if (cached?.filiais?.length) return cached;
  }

  if (!canUseDirectFirebird()) return null;
  ...
  try {
    ...
    await setJsonCache(cacheKey, access, {
      ttlSeconds: cacheTtlSeconds(),
      staleTtlSeconds: 0,
    });
    return access;
